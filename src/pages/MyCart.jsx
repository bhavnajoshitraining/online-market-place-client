...
const MyCart = () => {
    ...
    const loadMyCart = async () => {
        try {
            ...
            if (response.data) {
                ...
                setSnackbarMessage("Couldn't load cart items");
                setSnackbarSeverity("error");
                setOpenSnackbar(true);
    ...
    const placeOrderAndMakePayment = async () => {
        const data = cartData.map(item=> { return {productId: item.product.id, quantity: item.quantity}});
        console.log(data);
        try {
            const response = await axios({
                method: 'post',
              ...
              {cartData?.length>0 ?  <Grid container spacing={3}>
                    <Grid item xs={8}>
                        ...
                        {cartData?.length>0 && <Box sx={{ padding: 1, bgcolor: "#fff", display: "flex" }}>
                            ...
                            <Button onClick={()=> setPlaceOrder(true)} sx={{background: `linear-gradient(90deg, rgba(0,15,36,1) 12%, rgba(121,90,9,1) 65%, rgba(0,212,255,1) 100%);`, height: 50, width: 200}}  variant='contained'>Place Order</Button>
                        </Box>}
                    </Grid>
                    <Grid item xs={4}>
                        <Box sx={{ bgcolor: "#fff", paddingLeft: 2, paddingRight: 2 }}>
                            ...
                            {placeOrder && <Button onClick={()=> placeOrderAndMakePayment()} fullWidth sx={{background: `linear-gradient(90deg, rgba(131,58,180,1) 0%, rgba(253,29,29,1) 50%, rgba(252,176,69,1) 100%);`, height: 50}}  variant='contained'>Make Payment</Button>}
                        </Box>
                        </Box>
                    </Grid>
